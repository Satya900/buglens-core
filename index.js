  ...
  if (isShadowMode) {
    ...
    await octokit.pulls.createReview({
      ...
      comments: analysis.inlineComments
        ...
        .map((comment) => {
          // Prefer line+side (modern GitHub API) over position (deprecated).
          // line+side maps directly to the file line number — no off-by-one across hunks.
          if (comment.line > 0) {
            return {
              path: comment.path,
              line: comment.line,
          ...
          // Fallback to diff position for edge cases where line wasn't resolved
          return {
            path: comment.path,

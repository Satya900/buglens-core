...
//
// Deliberately NOT scope-aware (this file is regex-over-diff-lines like every
// other rule here, not an AST) — declarations and usages are matched across
// the whole diff, not per-function. To avoid flagging idiomatic short-name
// collisions (res/repo, prev/next, err/error, etc. are all mutually close
// under Levenshtein distance purely by being short common abbreviations, not
// because one is a typo of the other), fuzzy matching is restricted to
// longer, more "specific-looking" identifiers, with an explicit allowlist as
// a second line of defense. Found via two real false positives on a live
// PR: 'repo' (a function parameter) flagged as a typo of 'res' (a
// separately-declared fetch response), and 'prev' (a React setState-callback
// parameter) flagged as a typo of 'res'.
const MIN_FUZZY_LENGTH = 6; // shorter than this → never eligible for fuzzy matching

const SHORT_IDENTIFIER_ALLOWLIST = new Set([
  "res", "req", "repo", "rev", "ref", "prev", "next",
  "err", "error", "id", "ids", "ctx", "env", "arg", "args",
  "obj", "val", "key", "idx", "tmp", "cur", "url", "uri",
]);

function maxAllowedDistance(len) {
  // 6-9 chars: only a single-char typo counts. 10+: original slack.
  return len < 10 ? 1 : 2;
}
...
function detectVariableNameMismatch(file) {
  ...
  for (const { lineNumber, content } of addedLines) {
    ...
    for (const id of identifiers) {
      ...
      if (id.length < MIN_FUZZY_LENGTH) continue;
      if (SHORT_IDENTIFIER_ALLOWLIST.has(id)) continue;

      for (const name of declaredNames) {
        if (id === name) continue;
        if (name.length < MIN_FUZZY_LENGTH) continue;
        if (SHORT_IDENTIFIER_ALLOWLIST.has(name)) continue;

        const dist = levenshteinDistance(id, name);
        const allowedDist = Math.min(maxAllowedDistance(id.length), maxAllowedDistance(name.length));
        const key = `${id}|${name}`;

        // Flag if within the length-scaled distance and similar length — very likely a typo
        if (dist <= allowedDist && Math.abs(id.length - name.length) <= 3 && !reported.has(key)) {
          reported.add(key);
          findings.push(
            createRuleFinding(
              file.filename,

...
export async function sendReviewSummaryEmail({
  ...
  try {
    await resend.emails.send({
      from: FROM,
      to: toEmail,
      subject: `BugLens reviewed: ${prTitle} — ${decision}`,
      html: `
        <div style="font-family: monospace; max-width: 600px; margin: 0 auto; background: #0d1117; color: #e6edf3; padding: 32px; border-radius: 8px;">
          ...
          <p style="color: #8b949e; font-size: 13px; margin-bottom: 24px;">${repoFullName}</p>

          <div style="background: #161b22; border: 1px solid #30363d; border-radius: 6px; padding: 20px; margin-bottom: 20px;">
            <div style="font-size: 16px; font-weight: 600; margin-bottom: 8px;">${prTitle}</div>
            <div style="font-size: 20px; font-weight: 700; margin-bottom: 12px;">${decisionLabel}</div>
            <div style="font-size: 13px; color: #8b949e;">${findingsCount} finding(s) · ${riskSummary || "No critical issues"}</div>
          </div>
          ...
          <div style="display: flex; gap: 12px;">
            ...
            ${prUrl ? `<a href="${prUrl}" style="background: #21262d; color: #e6edf3; padding: 10px 20px; border-radius: 6px; text-decoration: none; font-size: 13px; border: 1px solid #30363d;">View on GitHub</a>` : ""}
          </div>

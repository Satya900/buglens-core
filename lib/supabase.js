...
/** Escape % _ \ so ilike is a case-insensitive literal match, not a pattern. */
...
/**
 * Resolve a profile by GitHub login. Exact match first, then case-insensitive
 * literal ilike (GitHub logins are case-insensitive; stored casing can differ).
 */
async function findProfileByGithubOwner(githubOwner, columns) {
  if (!githubOwner) return { data: null, error: null };
  ...
  return supabase
    .from("profiles")
    .select(columns)
    .ilike("github_username", escapeIlikeLiteral(githubOwner))
    .maybeSingle();
}
...
async function ensureRepoRecord({ repoFullName, githubOwner }) {
  ...
  if (githubOwner) {
    const { data: profile } = await findProfileByGithubOwner(githubOwner, "id");
...
export async function checkBillingEligibility(githubOwner) {
  try {
    const { data: profile, error } = await findProfileByGithubOwner(
      githubOwner,
      "id, subscription_tier, current_usage, usage_limit"
    );

    if (error) {
      // Any DB/network error — fail closed to protect billing
...
export async function incrementUserUsage(githubOwner) {
  try {
    const { data: profile } = await findProfileByGithubOwner(
      githubOwner,
      "id, current_usage, usage_limit, subscription_tier"
    );
...
export async function getUserEmailPrefs(githubOwner) {
  try {
    const { data, error } = await findProfileByGithubOwner(
      githubOwner,
...
export async function getUserEmail(githubOwner) {
  try {
    const { data, error } = await findProfileByGithubOwner(githubOwner, "email");
    if (error || !data) return null;

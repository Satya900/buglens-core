...
export function createModel(tier = "FREE") {
  ...
  const paidTier = new Set(["PRO", "BUSINESS"]);
  if (paidTier.has(String(tier || "").toUpperCase())) {
    // Paid tiers: primary model → fallback to free if primary is down
    const primary = createGeminiAdapter();
    const fallbacks = [or1, or2].filter(Boolean);
    console.log("[AI Provider] Using primary model (paid tier)");

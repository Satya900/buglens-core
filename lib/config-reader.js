...
//
// Supported fields:
//   strictness: "strict" | "balanced" | "relaxed" | "lenient"
//               ("lenient" is accepted as an alias for "relaxed")
...
const VALID_STRICTNESS = new Set(["strict", "balanced", "relaxed", "lenient"]);
const STRICTNESS_ALIASES = { lenient: "relaxed" };
...
function validate(raw, repoName) {
  ...
  if (raw.strictness !== undefined) {
    if (VALID_STRICTNESS.has(raw.strictness)) {
      config.strictness = STRICTNESS_ALIASES[raw.strictness] || raw.strictness;
    } else {
      logger.warn(

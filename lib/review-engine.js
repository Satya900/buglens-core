...
import { getStoredIndexFiles } from "./supabase.js";
...
/** Exported for tests / docs drift checks. */
export function getStrictnessConfig(strictness = "balanced") {
  return getStrictnessRules(strictness);
}
...
export async function analyzePullRequest({
  ...
  const knownFilesPromise =
    userId && repoFullName
      ? getStoredIndexFiles({ repoFullName, userId })
          .then((stored) => new Map(stored.map((f) => [f.file_path, f])))
          .catch(() => null)
      : Promise.resolve(null);

  // 1. Run walkthrough + codebase context + index load in parallel
  console.log(`Analyzing PR: "${pr.title}" — ${reviewableFiles.length} reviewable files`);
  const [walkthroughText, knownFilesByPath, codebaseContext] = await Promise.all([
    generateWalkthrough(model, pr, reviewableFiles),
    knownFilesPromise,
    codebaseContextPromise,
  ]);

  // Impact list is graph-grounded only (no free-text path invention).
  const impactedFiles = identifyImpactedFiles({ files, knownFilesByPath });

...
export function buildRepoProfile({ repoFullName, files, allFiles = [] }) {
  const capabilities = new Set();
  const contextFiles = allFiles.length > 0 ? allFiles : files;
  ...
  const riskLevel =
    criticalAreas.length >= 2 ? "high" : criticalAreas.length === 1 ? "medium" : "normal";
  ...
  return {
    ...
    criticalAreas,
    riskLevel,
    allFiles: allFiles.map((f) => f.filename), // Store filenames for reference
  };
}
